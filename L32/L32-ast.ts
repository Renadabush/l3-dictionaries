...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp|DictArray;
...
//2.2
export type DictExp = {tag:"DictExp"; var:SymbolSExp,val:CExp}
export type DictArray={tag:"DictArray"; val:DictExp[]}
...
export const makeLitExp = (val: SExpValue): LitExp =>
    ({tag: "LitExp", val: val});
//2.2
export const makeDictExp = (v1:SymbolSExp,v2:CExp):DictExp =>  ({tag: "DictExp", var:v1 ,val:v2});
export const makeDictArray = (v:DictExp[]):DictArray =>  ({tag: "DictArray", val:v});
...
export const isDictArray = (x: any): x is DictArray => x.tag === "DictArray";
// ========================================================
// Parsing
...
export const parseL32SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "quote" ? 
        isNonEmptyList<Sexp>(params) ? parseLitExp(first(params)) :makeFailure(`Bad quote exp: ${params}`) :
...
//2.2
export const isGoodDict = (elements: Sexp): elements is [string, Sexp][] =>
    isArray(elements) &&
    allT<NonEmptyList<Sexp>>(isNonEmptyList,elements) &&
    allT(isSexpSymbol, map(first, elements));
export const parseDictExp =(param:Sexp):Result<DictArray>=>{
    if(!(isGoodDict(param))){
        return makeFailure('Malformed syntax in "dict" expression');
    }
    const keys = map(b => makeSymbolSExp(b[0]), param);
    const valsResult = mapResult(parseL32CExp, map(second, param));
    return bind(
        mapv(valsResult, (vals: CExp[]) => zipWith(makeDictExp, keys, vals)),
        (v: DictExp[]) => makeOk(makeDictArray(v)) 
    );
    
    }

export const isSexpSymbol = (sexp: Sexp): sexp is string =>
        isString(sexp) &&
        !isNumericString(sexp) &&
        !isSexpString(sexp);
...
import { SymbolSExp } from "../L3/L3-value";
