...
import { isCompoundSExp, isEmptySExp, isSymbolSExp, makeCompoundSExp, makeEmptySExp, CompoundSExp, EmptySExp, Value, valueToString } from "./L31-value";
...
export const applyPrimitive = (proc: PrimOp, args: Value[]): Result<Value> =>
    ...
    //
    proc.op === "dict"? isCompoundSExp(args[0])? makeOk(args[0]):makeFailure(""):
    proc.op === "get"? args.length==2 ? isCompoundSExp(args[0])? get(args[0],args[1]):makeFailure(""):makeFailure(""):
    proc.op === "dict?"? isCompoundSExp(args[0]) ? makeOk(isDictPrim(args[0])): makeFailure(""):
    //
...
//
const get =(dict:CompoundSExp,index:Value):Result<Value> =>{
    if(isCompoundSExp(dict.val1)){
        if(valueEquals((dict.val1).val1,index))
            return makeOk((dict.val1).val2);
    }
    if(isEmptySExp(dict.val2)) 
        return makeFailure("Error");
    if(isCompoundSExp(dict.val2))
        return get(dict.val2,index);
    return makeFailure("Error");
...
//
//
const isDictPrim=(v:CompoundSExp| EmptySExp):boolean =>{
    if(isEmptySExp(v))
        return true;
    if(isCompoundSExp(v.val1)){
        if(isEmptySExp(v.val2))
            return true;
        else if(!(isCompoundSExp(v.val2)))
            return false;
        return (isDictPrim(v.val2));
    }
    return false;
...
const isPairPrim = (v: Value): boolean =>
    isCompoundSExp(v);
export const valueEquals = (v1: Value, v2: Value): boolean => {
    ...
    // Compare SymbolSExp
    if (isSymbolSExp(v1) && isSymbolSExp(v2)) return v1.val === v2.val;

    // Compare CompoundSExp recursively
    if (isCompoundSExp(v1) && isCompoundSExp(v2)) {
        return valueEquals(v1.val1, v2.val1) && valueEquals(v1.val2, v2.val2);
    }

    // Compare EmptySExp
    if (isEmptySExp(v1) && isEmptySExp(v2)) return true;
